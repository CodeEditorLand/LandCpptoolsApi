...
function isCppToolsTestExtension(
	extension: CppToolsTestApi | CppToolsTestExtension,
): extension is CppToolsTestExtension {
	return (<CppToolsTestExtension>extension).getTestApi !== undefined;
}

export async function getCppToolsTestApi(
	version: Version,
): Promise<CppToolsTestApi | undefined> {
	let cpptools: vscode.Extension<any> | undefined =
	...
	if (cpptools) {
		...
		if (isCppToolsTestExtension(extension)) {
			...
			if (version !== Version.v1) {
				if (!api.getVersion) {
					console.warn(
						`vscode-cpptools-api version ${version} requested, but is not available in the current version of the cpptools extension. Using version 1 instead.`,
					);
				} else if (version !== api.getVersion()) {
					console.warn(
						`vscode-cpptools-api version ${version} requested, but is not available in the current version of the cpptools extension. Using version ${api.getVersion()} instead.`,
					);
				}
			}
		} else {
			...
			if (version !== Version.v0) {
				console.warn(
					`vscode-cpptools-api version ${version} requested, but is not available in version 0.17.5 of the cpptools extension. Using version 0 instead.`,
				);
			}
		}
	} else {
