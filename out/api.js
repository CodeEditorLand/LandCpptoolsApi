...
function isLegacyCppToolsApi(api) {
	return (
		api &&
		api.registerCustomConfigurationProvider &&
		api.didChangeCustomConfiguration
	);
}
/**
			...
			if (isCppToolsExtension(extension)) {
				...
				try {
					...
					if (
						e &&
						e.message &&
						e.message.startsWith("Invalid version")
					) {
				...
				console.warn(
					"[vscode-cpptools-api] No cpptools API was found.",
				);
			}
		} else {
			console.warn(
				"[vscode-cpptools-api] C/C++ extension is not installed",
			);
		}
		return api;
	});
